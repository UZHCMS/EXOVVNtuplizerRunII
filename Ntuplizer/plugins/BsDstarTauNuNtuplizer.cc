...
bool BsDstarTauNuNtuplizer::fillBranches( edm::Event const & event, const edm::EventSetup& iSetup ){
  ...
  for(int iii = 0; iii < numOfch_pre; iii ++){
	...
	D0cand = aux.calculateIPvariables(extrapolator, D0_part, D0_vertex, closestVertex);
...
//			D0_part_rec->currentState().mass());
    ...
    Dscand = aux.calculateIPvariables(extrapolator, Ds_part, Ds_vertex, closestVertex);
...
//  int isRight_aS_n = 0;
  ...
  for(int ic=0; ic < (int)cands.size() && ic < 1; ic++){
    ...
    nBranches_->BsDstarTauNu_tau_lip.push_back(cands[ic].cand_tau.lip);
    nBranches_->BsDstarTauNu_tau_lips.push_back(cands[ic].cand_tau.lips);
    nBranches_->BsDstarTauNu_tau_pvip.push_back(cands[ic].cand_tau.pvip);
    nBranches_->BsDstarTauNu_tau_pvips.push_back(cands[ic].cand_tau.pvips);
    nBranches_->BsDstarTauNu_tau_fl3d.push_back(cands[ic].cand_tau.fl3d);
    nBranches_->BsDstarTauNu_tau_fls3d.push_back(cands[ic].cand_tau.fls3d);
    nBranches_->BsDstarTauNu_tau_alpha.push_back(cands[ic].cand_tau.alpha);
    ...
    nBranches_->BsDstarTauNu_B_lip.push_back(cands[ic].cand_b.lip);
    nBranches_->BsDstarTauNu_B_lips.push_back(cands[ic].cand_b.lips);
    nBranches_->BsDstarTauNu_B_pvip.push_back(cands[ic].cand_b.pvip);
    nBranches_->BsDstarTauNu_B_pvips.push_back(cands[ic].cand_b.pvips);
    nBranches_->BsDstarTauNu_B_fls3d.push_back(cands[ic].cand_b.fls3d);
    nBranches_->BsDstarTauNu_B_fl3d.push_back(cands[ic].cand_b.fl3d);
    nBranches_->BsDstarTauNu_B_alpha.push_back(cands[ic].cand_b.alpha);
    ...
    nBranches_->BsDstarTauNu_B_mindoca.push_back(aux.getMinDoca(allParticles4doc));
  ...
  nBranches_->BsDstarTauNu_mu1_dbiso.push_back(aux.MuonPFIso(muoncollection[0]));
