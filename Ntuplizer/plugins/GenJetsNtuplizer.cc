...
void GenJetsNtuplizer::fillBranches( edm::Event const & event, const edm::EventSetup& iSetup ){
  ...
  nBranches_->ngenJetsAK8 = 0;
  
  for (const pat::Jet &j : *genJetsAK8_) {
    // if (j.pt() < 20) continue;
    nBranches_->ngenJetsAK8++;
    ...
    // float visibleFraction = (j.energy()>0) ?(j.energy()-j.invisibleEnergy())/j.energy() : 0;
    // nBranches_->genJetNoNuAK8_pt           .push_back(j.pt()*visibleFraction);
    // nBranches_->genJetNoNuAK8_mass         .push_back(j.mass()*visibleFraction);
    // nBranches_->genJetNoNuAK8_e            .push_back(j.energy()*visibleFraction);
