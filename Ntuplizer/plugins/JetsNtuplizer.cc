...
void JetsNtuplizer::fillBranches( edm::Event const & event, const edm::EventSetup& iSetup ){
  ...
  event.getByToken(prunedjetInputToken_		, prunedjets_	);
  event.getByToken(softdropjetInputToken_	, softdropjets_	);
...
//	  printf("PJ with pt %5.1f , mass %+4.2f\n", prunedjet.pt(),  prunedjet.mass());
	  ...
	  nBranches_->jetAK8pruned_ssv    .push_back(prunedjet.bDiscriminator("simpleSecondaryVertexHighPurBJetTags")); //Obs! b-tag for fatjets not implemented yet. Recipe from Dinko when ready
	  nBranches_->jetAK8pruned_csv    .push_back(prunedjet.bDiscriminator("combinedInclusiveSecondaryVertexBJetTags"));
