...
void ElectronsNtuplizer::fillBranches( edm::Event const & event, const edm::EventSetup& iSetup ){
  ...
  for (const pat::Electron &ele : *electrons_) {
    ...
    // float eta = ele.caloPosition().eta();
 ...
 //              iso < isoCut && ele.dr03TkSumPt() < 5. && ele.gsfTrack()->trackerExpectedHitsInner().numberOfLostHits() <= 1 &&
 ...
 //              ele.hadronicOverEm() < 0.05 && ele.scSigmaIEtaIEta() < 0.03 && iso < isoCut && ele.gsfTrack()->trackerExpectedHitsInner().numberOfLostHits() <= 1
 ...
 //    if( !isHEEP ) continue;
    ...
    //std::cout << isHEEP << "/" << ele.trackIso() << std::endl;
