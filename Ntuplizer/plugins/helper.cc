...
reco::Track helper::fix_track(const reco::TrackRef& tk)
{
...
reco::Track helper::fix_track(const reco::Track *tk, double delta)
{
  //  double delta = 1e-8;
  ...
  return reco::Track(tk->chi2(), tk->ndof(), tk->referencePoint(), tk->momentum(), tk->charge(), cov, tk->algo(),
