...
//===================================================================================================================
JpsiKNtuplizer::JpsiKNtuplizer( edm::EDGetTokenT<pat::MuonCollection>    muonToken   ,
  ...
  , histGenWeights_ (histGenWeights)        
{
  ...
  if(verbose_){
    std::cout << "[JpsiKNtuplizer] runOnMC    = " << runOnMC_ << std::endl;
    std::cout << "[JpsiKNtuplizer] dzcut      = " << c_dz << std::endl;
  }
}
...
//===================================================================================================================
JpsiKNtuplizer::~JpsiKNtuplizer( void )
{
}



bool JpsiKNtuplizer::fillBranches( edm::Event const & event, const edm::EventSetup& iSetup ){

  if(verbose_) std::cout << "[JpsiKNtuplizer] ---------------- event, run, lumi = " << event.id().event() << " " << event.id().run() << " " << event.id().luminosityBlock() << "----------------" << std::endl;
    ...
    //    std::cout << "---------------- New event-------------"<< std::endl;
      for(size_t p=0; p < genParticles_->size(); ++p){
          if (abs((*genParticles_)[p].pdgId())==443 and abs((*genParticles_)[p].daughter(0)->pdgId())==13 ){ 
              ...
              motherID_ = abs((JpsiKNtuplizer::checkMom(&(*genParticles_)[p]))->pdgId());
  ...
  if(verbose_) std::cout << "[JpsiKNtuplizer] Trigger fired" << std::endl;
  ...
  if(verbose_) std::cout << "[JpsiKNtuplizer] At least 2 muons are found" << std::endl;
  ...
  if(verbose_) std::cout << "[JpsiKNtuplizer] J/psi found" << std::endl;
  ...
  if(verbose_) std::cout << "[JpsiKNtuplizer] J/psi soft muon ID passed" << std::endl;
  ...
  for(int iii = 0; iii < (int)pfcands.size(); iii ++){
    ...
    nBranches_->JpsiK_B_mass.push_back(b_part->currentState().mass());
    nBranches_->JpsiK_B_vprob.push_back( TMath::Prob(b_vertex->chiSquared(), b_vertex->degreesOfFreedom()) ); //b_part->currentState().mass());
    nBranches_->JpsiK_B_lip.push_back(Bcand.lip);
    nBranches_->JpsiK_B_lips.push_back(Bcand.lips);
    nBranches_->JpsiK_B_pvip.push_back(Bcand.pvip);
    nBranches_->JpsiK_B_pvips.push_back(Bcand.pvips);
    nBranches_->JpsiK_B_fls3d.push_back(Bcand.fls3d);
    nBranches_->JpsiK_B_fl3d.push_back(Bcand.fl3d);
    nBranches_->JpsiK_B_alpha.push_back(Bcand.alpha);    
    nBranches_->JpsiK_B_vx.push_back(b_vertex->vertexState().position().x());
    nBranches_->JpsiK_B_vy.push_back(b_vertex->vertexState().position().y());
    nBranches_->JpsiK_B_vz.push_back(b_vertex->vertexState().position().z());

    nBranches_->JpsiK_pi_pt.push_back(pf.pt());
    nBranches_->JpsiK_pi_eta.push_back(pf.eta());
    nBranches_->JpsiK_pi_phi.push_back(pf.phi());
    nBranches_->JpsiK_pi_mass.push_back(pf.mass());
    nBranches_->JpsiK_pi_q.push_back(pf.charge());
    nBranches_->JpsiK_pi_doca3d.push_back(attr.doca3d);
    nBranches_->JpsiK_pi_doca3de.push_back(attr.doca3de);
    nBranches_->JpsiK_pi_doca2d.push_back(attr.doca2d);
    nBranches_->JpsiK_pi_doca2de.push_back(attr.doca2de);
    nBranches_->JpsiK_pi_dz.push_back(attr.dz);
    nBranches_->JpsiK_pi_near_dz.push_back(attr.near_dz);
    nBranches_->JpsiK_pi_isAssociate.push_back(attr.isAssociate);
    nBranches_->JpsiK_pi_pvAssociationQuality.push_back(attr.pvAssociationQuality);
  ...
  nBranches_->JpsiK_mu1_pt = muoncollection[mcidx_mu1].pt();
  nBranches_->JpsiK_mu1_eta = muoncollection[mcidx_mu1].eta();
  nBranches_->JpsiK_mu1_phi = muoncollection[mcidx_mu1].phi();
  nBranches_->JpsiK_mu1_mass = muoncollection[mcidx_mu1].mass();
  nBranches_->JpsiK_mu1_q = muoncollection[mcidx_mu1].charge();
  nBranches_->JpsiK_mu1_isLoose = muoncollection[mcidx_mu1].isLooseMuon();
  nBranches_->JpsiK_mu1_isTight = muoncollection[mcidx_mu1].isTightMuon(closestVertex);
  nBranches_->JpsiK_mu1_isPF = muoncollection[mcidx_mu1].isPFMuon();
  nBranches_->JpsiK_mu1_isGlobal = muoncollection[mcidx_mu1].isGlobalMuon();
  nBranches_->JpsiK_mu1_isTracker = muoncollection[mcidx_mu1].isTrackerMuon();
  nBranches_->JpsiK_mu1_isSoft = muoncollection[mcidx_mu1].isSoftMuon(closestVertex);
  nBranches_->JpsiK_mu1_vx = muoncollection[mcidx_mu1].vx();
  nBranches_->JpsiK_mu1_vy = muoncollection[mcidx_mu1].vy();
  nBranches_->JpsiK_mu1_vz = muoncollection[mcidx_mu1].vz();
  nBranches_->JpsiK_mu1_dbiso = aux.MuonPFIso(muoncollection[mcidx_mu1]);
  
  nBranches_->JpsiK_mu2_pt = muoncollection[mcidx_mu2].pt();
  nBranches_->JpsiK_mu2_eta = muoncollection[mcidx_mu2].eta();
  nBranches_->JpsiK_mu2_phi = muoncollection[mcidx_mu2].phi();
  nBranches_->JpsiK_mu2_mass = muoncollection[mcidx_mu2].mass();
  nBranches_->JpsiK_mu2_q = muoncollection[mcidx_mu2].charge();
  nBranches_->JpsiK_mu2_isLoose = muoncollection[mcidx_mu2].isLooseMuon();
  nBranches_->JpsiK_mu2_isTight = muoncollection[mcidx_mu2].isTightMuon(closestVertex);
  nBranches_->JpsiK_mu2_isPF = muoncollection[mcidx_mu2].isPFMuon();
  nBranches_->JpsiK_mu2_isGlobal = muoncollection[mcidx_mu2].isGlobalMuon();
  nBranches_->JpsiK_mu2_isTracker = muoncollection[mcidx_mu2].isTrackerMuon();
  nBranches_->JpsiK_mu2_isSoft = muoncollection[mcidx_mu2].isSoftMuon(closestVertex);
  nBranches_->JpsiK_mu2_vx = muoncollection[mcidx_mu2].vx();
  nBranches_->JpsiK_mu2_vy = muoncollection[mcidx_mu2].vy();
  nBranches_->JpsiK_mu2_vz = muoncollection[mcidx_mu2].vz();
  nBranches_->JpsiK_mu2_dbiso = aux.MuonPFIso(muoncollection[mcidx_mu2]);

  nBranches_->JpsiK_PV_vx = vertices_->begin()->position().x();
  nBranches_->JpsiK_PV_vy = vertices_->begin()->position().y();
  nBranches_->JpsiK_PV_vz = vertices_->begin()->position().z();

  nBranches_->JpsiK_bbPV_vx = closestVertex.position().x();
  nBranches_->JpsiK_bbPV_vy = closestVertex.position().y();
  nBranches_->JpsiK_bbPV_vz = closestVertex.position().z();
  nBranches_->JpsiK_bbPV_chi2 = closestVertex.chi2();
  nBranches_->JpsiK_bbPV_ndof = closestVertex.ndof();
  nBranches_->JpsiK_bbPV_rho = closestVertex.position().Rho();

  nBranches_->JpsiK_Jpsi_pt = jpsi_part->currentState().globalMomentum().perp();
  nBranches_->JpsiK_Jpsi_eta = jpsi_part->currentState().globalMomentum().eta();
  nBranches_->JpsiK_Jpsi_phi = jpsi_part->currentState().globalMomentum().phi();
  nBranches_->JpsiK_Jpsi_mass = jpsi_part->currentState().mass();
  nBranches_->JpsiK_Jpsi_vprob = TMath::Prob(jpsi_part->chiSquared(), jpsi_part->degreesOfFreedom());

  particle_cand JPcand;
  JPcand = aux.calculateIPvariables(extrapolator, jpsi_part, jpsi_vertex, closestVertex);

  nBranches_->JpsiK_Jpsi_lip = JPcand.lip;
  nBranches_->JpsiK_Jpsi_lips = JPcand.lips;
  nBranches_->JpsiK_Jpsi_pvip = JPcand.pvip;
  nBranches_->JpsiK_Jpsi_pvips = JPcand.pvips;
  nBranches_->JpsiK_Jpsi_fl3d = JPcand.fl3d;
  nBranches_->JpsiK_Jpsi_fls3d = JPcand.fls3d;
  nBranches_->JpsiK_Jpsi_alpha = JPcand.alpha;
  nBranches_->JpsiK_Jpsi_maxdoca = aux.getMaxDoca(muonParticles);
  nBranches_->JpsiK_Jpsi_mindoca = aux.getMinDoca(muonParticles);
  nBranches_->JpsiK_Jpsi_vx = jpsi_vertex->vertexState().position().x();
  nBranches_->JpsiK_Jpsi_vy = jpsi_vertex->vertexState().position().y();
  nBranches_->JpsiK_Jpsi_vz = jpsi_vertex->vertexState().position().z();  
  ...
  nBranches_->JpsiK_q2_gen = q2_gen.M2();
  nBranches_->JpsiK_nBc = nBc;
  nBranches_->JpsiK_B_pt_gen = pB_gen.Pt();
  nBranches_->JpsiK_B_eta_gen = pB_gen.Eta();
  nBranches_->JpsiK_B_phi_gen = pB_gen.Phi();
  nBranches_->JpsiK_B_mass_gen = pB_gen.M();
  ...
  nBranches_->JpsiK_genPV_vx = genvertex.x();
  nBranches_->JpsiK_genPV_vy = genvertex.y();
  nBranches_->JpsiK_genPV_vz = genvertex.z();

  nBranches_->JpsiK_genSV_vx = genvertex_sv.x();
  nBranches_->JpsiK_genSV_vy = genvertex_sv.y();
  nBranches_->JpsiK_genSV_vz = genvertex_sv.z();

  nBranches_->JpsiK_ngenmuons = gen_nr_mu.size() + gen_jpsi_mu.size();
  nBranches_->JpsiK_isgenmatched = (int)flag_jpsi_match;
...
const reco::Candidate*  JpsiKNtuplizer::checkMom(const reco::Candidate * candMom){
